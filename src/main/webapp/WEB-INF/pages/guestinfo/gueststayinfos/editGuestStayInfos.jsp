...
<html>
<head>
<title>Edit <fmt:message key="guestinfo.title"/> <fmt:message key="gueststayinfo.title"/></title>
</head>
<body>
<div id="contentarea">      
	...
	<div id="content">
		...
		<div class="navitem"><a class="button" href="${pageContext.request.contextPath}/selectGuestInfo?idKey=${guestinfo_id}&"><span><img src="images/icons/back.gif" /><fmt:message key="navigation.back"/></span></a></div>
		<form:form action="${pageContext.request.contextPath}/saveGuestInfoGuestStayInfos" method="POST" modelAttribute="gueststayinfo">
				...
				<tbody>
					<tr>
						...
						<td>
							<c:choose>
								<c:when test='${newFlag}' >
							<form:input id="gueststayinfo_id" path="id" cssStyle="width:300px;"/>
							<script type="text/javascript">Spring.addDecoration(new Spring.ElementDecoration({elementId : "gueststayinfo_id",widgetType : "dijit.form.NumberTextBox",widgetAttrs : {promptMessage: "<fmt:message key="gueststayinfo.id.help"/>", constraints : {places:0}}})); </script>
								</c:when>
								<c:otherwise>
							${gueststayinfo.id}
						&nbsp;
									<form:hidden id="gueststayinfo_id" path="id"/>
								</c:otherwise>
							</c:choose>
						</td>
					</tr>
			...
			<script type="text/javascript">Spring.addDecoration(new Spring.ValidateAllDecoration({elementId:'save', event:'onclick'}));</script>
				<input type="hidden" name="guestinfo_id" value="${guestinfo_id}" >
